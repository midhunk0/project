...
const AdminRecruiters = () => {
  ...
  const handleReject = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:8080/api/recruiters/recruiterRequest/${selectedRecruiter._id}`, {
        action: action,
      })
    } catch (err) {
      console.log(err)
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      await Promise.all([
        axios.post(`http://localhost:8080/api/recruiters/recruitermatch/${selectedRecruiter._id}`),
        axios.put(`http://localhost:8080/api/recruiters/recruiterRequest/${selectedRecruiter._id}`, {
          action: action,
        })
      ]);
